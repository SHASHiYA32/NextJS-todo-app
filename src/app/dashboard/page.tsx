...
import { currentUser, notes, notifications, progressMetrics, tasks } from "@/mock/data";
...
export default function DashboardPage() {
  ...
  const upcomingDeadlines = tasks.filter((task) => task.status !== "Completed").slice(0, 3);

  return (
    <div className="space-y-6 sm:space-y-8 pb-8">
      {/* Welcome Section with Gradient */}
      <section className="grid gap-4 sm:gap-6 lg:grid-cols-[1.6fr_1fr]">
        <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-gradient-to-br from-primary/20 via-card/90 to-card/90 p-6 sm:p-8 shadow-xl shadow-black/5 overflow-hidden relative">
          ...
          <CardHeader className="p-0 relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
              <div>
                ...
                <CardTitle className="mt-2 sm:mt-3 text-2xl sm:text-4xl font-bold bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">Hello, {currentUser.name}</CardTitle>
              ...
              <div className="rounded-full bg-gradient-to-br from-primary/40 to-primary/20 p-3 sm:p-4 text-primary flex-shrink-0">
          ...
          <CardContent className="grid gap-4 sm:gap-6 pt-6 sm:pt-8 lg:grid-cols-2 relative z-10">
            <div className="grid gap-3 rounded-xl sm:rounded-[1.75rem] border border-border/70 bg-gradient-to-br from-emerald-500/10 to-background/80 p-4 sm:p-6 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex h-12 sm:h-14 w-12 sm:w-14 items-center justify-center rounded-3xl bg-emerald-500/20 text-emerald-400 flex-shrink-0">
            ...
            <div className="grid gap-3 rounded-xl sm:rounded-[1.75rem] border border-border/70 bg-gradient-to-br from-sky-500/10 to-background/80 p-4 sm:p-6 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-4">
                <div>
                ...
                <div className="flex h-12 sm:h-14 w-12 sm:w-14 items-center justify-center rounded-3xl bg-sky-500/20 text-sky-400 flex-shrink-0">
        ...
        <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-card/90 p-4 sm:p-8 shadow-lg shadow-black/5">
          ...
          <CardContent className="p-0 grid gap-4 sm:gap-6 lg:grid-cols-2">
            {progressMetrics.map((metric) => (
              <div key={metric.title} className="rounded-xl sm:rounded-[1.5rem] border border-border/70 bg-gradient-to-br from-primary/5 to-background/80 p-4 sm:p-5">
          ...
          <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-card/90 p-4 sm:p-6 shadow-lg shadow-black/5">
            ...
            <div className="space-y-3">
              {upcomingDeadlines.map((task) => (
                <div key={task.id} className="rounded-2xl border border-border/70 bg-gradient-to-r from-primary/5 to-transparent p-3 sm:p-4 hover:border-primary/50 transition">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-xs sm:text-sm text-muted-foreground flex-shrink-0">{task.deadline}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-card/90 p-4 sm:p-6 shadow-lg shadow-black/5">
            ...
            <div className="grid gap-3">
              {notes.slice(0, 3).map((note) => (
                <div key={note.id} className="rounded-2xl bg-gradient-to-r from-primary/5 to-transparent p-3 sm:p-4 border border-border/50 hover:border-primary/30 transition">
      ...
      <section className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-gradient-to-br from-blue-500/10 to-card/90 p-4 sm:p-6 shadow-lg shadow-black/5 hover:shadow-xl transition group cursor-pointer">
          <div className="flex items-start justify-between gap-3">
            <div>
            ...
            <ArrowRight className="h-5 w-5 text-muted-foreground group-hover:text-primary transition flex-shrink-0 mt-1" />
          </div>
        </Card>

        <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-gradient-to-br from-purple-500/10 to-card/90 p-4 sm:p-6 shadow-lg shadow-black/5 hover:shadow-xl transition group cursor-pointer">
          <div className="flex items-start justify-between gap-3">
            <div>
        ...
        <Card className="rounded-2xl sm:rounded-[2rem] border border-border/80 bg-gradient-to-br from-rose-500/10 to-card/90 p-4 sm:p-6 shadow-lg shadow-black/5 hover:shadow-xl transition group cursor-pointer sm:col-span-2 lg:col-span-1">
          <div className="flex items-start justify-between gap-3">
            <div>
